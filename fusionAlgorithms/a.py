...
def fusion(a, b, A, B):
    ...
    S_0 = 0.3*(np.linalg.cholesky(inv(mutual_covariance(np.copy(A), np.copy(B)) +1e-2*np.identity(2))).T).reshape(4, )
    prob_constraint(S_0)
...
def update_determinants(dets, covs, n):
    for i in range(n):
        dets[i].append(np.linalg.det(covs[i]))

def get_edge_list(graph):
    edges = []
    for node0 in graph:
        for node1 in graph[node0][0]:
            edge0 = [node0, node1]
            edge1 = [node1, node0]
            
            duplicate = edge0 in edges or edge1 in edges
            if not duplicate:
                edges.append(edge0)
...
def run_fusion(mus, covs, graph, n_agents, iter=50):
    edge_list = get_edge_list(graph)

    dets = {}
    for i in range(n_agents):
        dets[i] = [np.linalg.det(covs[i])]
    ...
    for i in tqdm(range(iter)):
        for edge in edge_list:
            ...
            C_fus, x_fus = fusion(x_a, x_b, np.copy(C_a), np.copy(C_b))
            mus[n] = x_fus.reshape(mus[n].shape)
            mus[j] = x_fus.reshape(mus[j].shape)
            
            covs[n] = C_fus.reshape(covs[n].shape)
            covs[j] = C_fus.reshape(covs[j].shape)

        update_determinants(dets, covs, n_agents)
